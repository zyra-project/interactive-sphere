...
| 2/M | Live single-row run surfaced `migration-telemetry: emit returned 403` — Node fetch doesn't set Origin by default, and `/api/ingest`'s same-origin check rejects empty origins. Stamp `Origin: <serverUrl>` on every emit; three regression tests pin the header. |
| 2/N | `check-pages-bindings` audit tidy-up: add R2 S3-API creds (`R2_ACCESS_KEY_ID` / `R2_SECRET_ACCESS_KEY` / `R2_S3_ENDPOINT`) and `FEEDBACK_ADMIN_TOKEN` to the manifest so they stop showing as "extra"; extend the `CATALOG_KV` hint to call out the binding-name-vs-namespace-name foot-gun (a live operator audit revealed `env.CATALOG_KV` was undefined because the binding name was set to the raw namespace id). |
| 2/O | Live single-row run hit `TUS PATCH unreachable: Response body object should not be disturbed or locked` — Node's undici can't replay a streaming body on Cloudflare's regional-routing redirects. Buffer the source bytes before PATCH (matches Phase 1b's `runUpload` precedent). `uploadToStream` body type widened to `Uint8Array \| ReadableStream`; new `drainStream` helper enforces a per-row buffer cap with pre-flight + mid-stream checks. |
| 2/P | Bump the per-row buffer cap from 256 MB (inherited Phase-1b thinking) to 2 GiB. The migration bypasses the Pages Function entirely, so the Worker 128 MB memory limit doesn't apply — and the operator confirmed the SOS catalog has ~10-15 rows above 256 MB. Operator's laptop sees ≤ 2 GiB peak (sequential migration, one row resident at a time). |
| 2/Q | Round-2 Copilot fixes: (1) docstring-vs-behavior mismatch in `drainStream` — claimed to verify advertised-vs-actual length, didn't; added the post-read length check. (2) `--id` mode bypasses the `status=published` filter; rewrote `buildPlan` docstring to honestly describe both paths and the legitimate reasons --id is permissive. (3) Updated PR title from H-only to phase-level scope. |
| 2/R | Live single-row playback verification revealed the standard Stream plan caps rendition output at 1080p (a 4K equirectangular source rendered to a 2160x1080 top rendition). New `terraviz rollback-stream <id> --to-vimeo=<vimeo_id>` subcommand automates per-row rollback (PATCH first as commit point, then DELETE the Stream asset). Added `deleteStreamAsset` library helper. Runbook gains a "Known limitations: Stream rendition ceiling" subsection and updated rollback procedure. |
